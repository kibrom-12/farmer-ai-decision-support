...
def load_fertilizer_data():
    return pd.read_csv(APP_DIR / "TAMASA_fertilizer_modeling_table.csv")
...
soil_n = st.sidebar.number_input(
    ...
    step=0.01,
    format="%.2f",
    help="Total soil nitrogen as percent (%)."
)
...
soil_p = st.sidebar.number_input(
    ...
    value=7.0,
    step=0.1,
    help="Available soil phosphorus in ppm."
)

st.sidebar.caption(
    "Fertilizer AI uses soil pH, total N and available P. "
    "Soil K is intentionally omitted because complete soil-K measurements "
    "were not available in the TAMASA dataset."
)
...
if st.sidebar.button("Run Analysis"):
    with st.spinner("Analyzing farm location data & running AI models..."):
        ...
        st.header("🧪 Soil Nutrient & Fertilizer Decision Support")

        st.write("Current soil measurements entered for this analysis:")
        ...
        with npk_col2:
            st.metric("Total N (%)", f"{soil_n:.2f}")

        with npk_col3:
            st.metric("Available P (ppm)", f"{soil_p:.1f}")
        ...
        try:
            ...
            with f_col1:
                st.success(
                    ...
                    f"{fert_result['recommended_strategy']}"
                )

            with f_col2:
                st.info(
                    f"**Decision confidence:** "
                    f"{fert_result['confidence']}"
                )

            st.metric(
                "Estimated Yield Under Recommended Strategy",
                f"{fert_result['estimated_yield_kg_ha']:,.0f} kg/ha",
            )

            st.subheader("📊 Fertilizer Strategy Comparison")

            strategy_df = pd.DataFrame({
                "Fertilizer Strategy": fert_result["ranked_strategies"],
                "Estimated Yield (kg/ha)": [
                    fert_result["estimated_yield_by_strategy"][name]
                    for name in fert_result["ranked_strategies"]
                ],
            })

            strategy_df["Estimated Yield (kg/ha)"] = (
                strategy_df["Estimated Yield (kg/ha)"].round(0)
            )
            ...
            for item in fert_result["soil_diagnosis"]:
                st.write(f"• {item}")

            st.subheader("💡 Why this recommendation?")
            st.write(fert_result["reason"])

            st.warning(fert_result["warning"])

        except Exception as exc:
            st.error(
                "The fertilizer decision engine could not run with the "
                f"current TAMASA data: {exc}"
            )

        st.subheader("🔌 Future IoT Integration")

        st.caption(
            "NPK and pH measurements can later be supplied by calibrated "
            "soil sensors through an IoT device such as an ESP32. The same "
            "measurements can feed the validated fertilizer decision model."
        )
